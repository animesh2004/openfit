...
// runllm MCP server — exposes runllm's hardware scan + model-fit logic as
...
const server = new Server(
  { name: "runllm", version: pkg.version },
  { capabilities: { tools: {} } }
);
...
async function main() {
  ...
  console.error(`runllm MCP server v${pkg.version} running on stdio`);
}

main().catch((err) => {
  console.error("runllm MCP server failed:", err);
  process.exit(1);
});
